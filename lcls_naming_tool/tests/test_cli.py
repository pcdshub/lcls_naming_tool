...
from lcls_naming_tool.lcls_naming_tool import (
    constituent_component_is_valid,
    functional_component_is_valid,
    fungible_is_valid,
    get_version,
    increment_is_valid,
    load_taxons,
...
@pytest.mark.parametrize(
    "test_name, expected", [("MR2K4", True), ("TV3L2", True), ("XCS", False)]
)
...
@pytest.mark.parametrize(
    "test_name, expected", [("XYZ", False), ("ABCDEF", False), ("LD10", True)]
)
...
@pytest.mark.parametrize(
    "test_name, expected", [("CBS", False), ("CLF", True), ("345", False)]
)
...
@pytest.mark.parametrize(
    "test_name, expected", [("02", True), ("A36", False), ("3971", True)]
)
...
@pytest.mark.parametrize(
    "test_name, expected", [("_PCTRLSPRBCK", False), ("PUMPSIZE", True), ("7ULM", True)]
)
...
@pytest.mark.parametrize(
    "test_name, expected",
    [("MR2K4:PIP:03", True), ("TV3L2:KBO:PMONRAW", False), ("AT1L9:DG3:GCC", False)],
)
...
@pytest.mark.parametrize(
    "test_name, expected",
    [
        ("MR2K4:GGG:PIP:01:PUMPSIZE", False),
        ("TV3L2:KBO:MRT:08:PMONRAW", False),
        ("AT1L9:DG3:GIGE:04:3XYZ", False),
    ],
)
